...
def flash_attn_3_func(q, k, v, causal=True):
    # Shim: FA3/FA2 not available on 4090, use PyTorch SDPA instead.
    # Input: (batch, seqlen, nheads, head_dim) -> SDPA wants (batch, nheads, seqlen, head_dim)
    q = q.transpose(1, 2)
    k = k.transpose(1, 2)
    v = v.transpose(1, 2)
    y = F.scaled_dot_product_attention(q, k, v, attn_mask=None, is_causal=causal,
                                       enable_gqa=(q.size(1) != k.size(1)))
    return y.transpose(1, 2)  # back to (batch, seqlen, nheads, head_dim)
class Hyperparameters:
    ...
    # ── Training schedule ───────────────────────────────────────────────
    val_batch_size = int(os.environ.get("VAL_BATCH_SIZE", 524_288))  # [UPSTREAM]
    ...
    eval_seq_len = int(os.environ.get("EVAL_SEQ_LEN", 2048))  # [NOVEL]
    max_wallclock_seconds = float(os.environ.get("MAX_WALLCLOCK_SECONDS", 600.0))  # [UPSTREAM]

    # ── Architecture ────────────────────────────────────────────────────
    qk_gain_init = float(os.environ.get("QK_GAIN_INIT", 1.5))  # [UPSTREAM]
    vocab_size = int(os.environ.get("VOCAB_SIZE", 1024))  # [UPSTREAM]
    num_layers = int(os.environ.get("NUM_LAYERS", 11))  # [TUNED] upstream=9, +2 layers fits budget
    num_kv_heads = int(os.environ.get("NUM_KV_HEADS", 4))  # [UPSTREAM]
    model_dim = int(os.environ.get("MODEL_DIM", 512))  # [UPSTREAM]
    num_heads = int(os.environ.get("NUM_HEADS", 8))  # [UPSTREAM]
    mlp_mult = float(os.environ.get("MLP_MULT", 3.0))  # [TUNED] upstream=2, wider MLPs
    tie_embeddings = bool(int(os.environ.get("TIE_EMBEDDINGS", "1")))  # [UPSTREAM]
    rope_base = float(os.environ.get("ROPE_BASE", 10000.0))  # [UPSTREAM]
    logit_softcap = float(os.environ.get("LOGIT_SOFTCAP", 30.0))  # [UPSTREAM]

    # ── Optimizer ───────────────────────────────────────────────────────
    embed_lr = float(os.environ.get("EMBED_LR", 0.6))  # [UPSTREAM]
    head_lr = float(os.environ.get("HEAD_LR", 0.008))  # [UPSTREAM]
    tied_embed_lr = float(os.environ.get("TIED_EMBED_LR", 0.035))  # [TUNED] upstream=0.05
    tied_embed_init_std = float(os.environ.get("TIED_EMBED_INIT_STD", 0.005))  # [UPSTREAM]
    matrix_lr = float(os.environ.get("MATRIX_LR", 0.025))  # [TUNED] upstream=0.04
    scalar_lr = float(os.environ.get("SCALAR_LR", 0.025))  # [TUNED] upstream=0.04
    muon_momentum = float(os.environ.get("MUON_MOMENTUM", 0.99))  # [TUNED] upstream=0.95
    muon_backend_steps = int(os.environ.get("MUON_BACKEND_STEPS", 5))  # [UPSTREAM]
    muon_momentum_warmup_start = float(os.environ.get("MUON_MOMENTUM_WARMUP_START", 0.92))  # [TUNED] upstream=0.85
    muon_momentum_warmup_steps = int(os.environ.get("MUON_MOMENTUM_WARMUP_STEPS", 1500))  # [TUNED] upstream=500
    beta1 = float(os.environ.get("BETA1", 0.9))  # [UPSTREAM]
    beta2 = float(os.environ.get("BETA2", 0.95))  # [UPSTREAM]
    adam_eps = float(os.environ.get("ADAM_EPS", 1e-8))  # [UPSTREAM]
    grad_clip_norm = float(os.environ.get("GRAD_CLIP_NORM", 0.3))  # [TUNED] upstream=0.0 (no clipping)
    muon_beta2 = float(os.environ.get("MUON_BETA2", 0.95))  # [NOVEL]
    muon_wd = float(os.environ.get("MUON_WD", 0.04))  # [NOVEL]
    adam_wd = float(os.environ.get("ADAM_WD", 0.04))  # [NOVEL]

    # ── NOVEL: Weight averaging ─────────────────────────────────────────
    swa_enabled = bool(int(os.environ.get("SWA_ENABLED", "1")))  # [NOVEL]
    swa_every = int(os.environ.get("SWA_EVERY", 50))  # [NOVEL]
    lawa_enabled = bool(int(os.environ.get("LAWA_ENABLED", "0")))  # [NOVEL]
    lawa_k = int(os.environ.get("LAWA_K", 10))  # [NOVEL]
    ...
    xsa_last_n = int(os.environ.get("XSA_LAST_N", 11))  # [NOVEL] XSA on all layers

    # ── NOVEL: Architecture tweaks ──────────────────────────────────────
    rope_dims = int(os.environ.get("ROPE_DIMS", 16))  # [NOVEL] reduced RoPE dims
    ln_scale = bool(int(os.environ.get("LN_SCALE", "1")))  # [NOVEL] learnable LN scales
    gated_attention = bool(int(os.environ.get("GATED_ATTENTION", "0")))  # [NOVEL] risky, off by default
    value_residual = bool(int(os.environ.get("VALUE_RESIDUAL", "0")))  # [NOVEL] VRL with sigmoid gates, risky

    # ── NOVEL: Value embeddings ─────────────────────────────────────────
    ve_enabled = bool(int(os.environ.get("VE_ENABLED", "1")))  # [NOVEL]
    ve_dim = int(os.environ.get("VE_DIM", 128))  # [NOVEL]
    ve_layers = os.environ.get("VE_LAYERS", "9,10")  # [NOVEL] only last 2 layers

    # ── NOVEL: Quantization ─────────────────────────────────────────────
    qat_enabled = bool(int(os.environ.get("QAT_ENABLED", "0")))  # [NOVEL]
    late_qat_threshold = float(os.environ.get("LATE_QAT_THRESHOLD", 0.15))  # [NOVEL]
    dtg_enabled = bool(int(os.environ.get("DTG_ENABLED", "0")))  # [NOVEL]
    gptq_calib_batches = int(os.environ.get("GPTQ_CALIB_BATCHES", 256))  # [NOVEL]
    gptq_block_size = int(os.environ.get("GPTQ_BLOCK_SIZE", 128))  # [NOVEL]

    # ── NOVEL: Attention residuals (SHELVED — torch.compile breaks) ─────
    attn_res = bool(int(os.environ.get("ATTN_RES", "0")))  # [NOVEL]
    ...
    ttt_momentum = float(os.environ.get("TTT_MOMENTUM", 0.9))  # [NOVEL]
    ttt_grad_clip = float(os.environ.get("TTT_GRAD_CLIP", 1.0))  # [NOVEL]
...
class BlockAttnRes(nn.Module):
    """Block Attention Residuals (arxiv 2603.15031): layers are grouped into blocks,
    with standard residuals within blocks and softmax attention across blocks.
    Uses a Python list of slot tensors — write is a pointer swap (free),
    torch.stack only happens inside aggregate. Compile-friendly + autograd-safe."""
    def __init__(self, num_layers: int, block_size: int, dim: int):
        super().__init__()
        self.block_size = block_size
        self.num_blocks = (num_layers + block_size - 1) // block_size
        self.max_slots = self.num_blocks + 2  # embedding + completed blocks + partial
        self.queries = nn.Parameter(torch.randn(num_layers + 1, dim) * 0.02)
        masks = []
        for i in range(num_layers + 1):
            num_completed = min(i, num_layers) // block_size
            num_valid = num_completed + 2
            mask = torch.zeros(self.max_slots, dtype=torch.bool)
            mask[:num_valid] = True
            masks.append(mask)
        self.register_buffer('masks', torch.stack(masks), persistent=False)

    def aggregate(self, slots: list[Tensor], layer_idx: int) -> Tensor:
        """Attend over slot tensors.
        slots: list of max_slots tensors, each [B, T, D] (unused slots are zeros).
        layer_idx: which layer is requesting input.
        """
        buf = torch.stack(slots)  # [max_slots, B, T, D]
        mask = self.masks[layer_idx]
        K = F.rms_norm(buf, (buf.size(-1),))
...
class GPT(nn.Module):
    def __init__(
        ...
        super().__init__()
        self._attn_res_enabled = attn_res
        ...
        self.attn_res = BlockAttnRes(num_layers, block_size=attn_res_block_size, dim=model_dim) if attn_res else None
        self._init_weights()
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.attn_res is not None:
            bs = self.attn_res.block_size
            ms = self.attn_res.max_slots
            zeros = torch.zeros_like(x)
            # Python list of slot tensors — slot 0 = embedding, rest start as zeros
            slots: list[Tensor] = [x] + [zeros] * (ms - 1)
            partial = torch.zeros_like(x)
            next_slot = 1
            for i in range(n):
                slots[next_slot] = partial
                h = self.attn_res.aggregate(slots, i)
                ve = self._get_ve(i, input_ids, ve_cache)
                x_out, raw_v = self.blocks[i](h, x0,
                    self.qo_bank[i], self.kv_bank[i], self.kv_bank[n + i],
                    self.qo_bank[n + i], self.mlp_up_bank[i], self.mlp_down_bank[i],
                    v_embed=ve, v0=v0)
                if v0 is None and raw_v is not None:
                    v0 = raw_v
                partial = partial + (x_out - h)
                if (i + 1) % bs == 0:
                    slots[next_slot] = partial
                    next_slot += 1
                    partial = torch.zeros_like(x)
            slots[next_slot] = partial
            x = self.attn_res.aggregate(slots, n)
        else:
...
def eval_val_ttt(
    ...
    """Score-first Test-Time Training: score each chunk under inference_mode,
    then adapt on already-scored chunks. Legal per challenge rules."""
    seq_len = eval_seq_len or args.train_seq_len
    chunk_size = args.ttt_chunk_tokens
    total_tokens = val_tokens.numel() - 1
    # Split val into non-overlapping chunks
    chunk_starts = list(range(0, total_tokens - seq_len, chunk_size))
    if not chunk_starts:
        chunk_starts = [0]

    # Save initial model state for reporting
    model.eval()
    ...
    # SGD optimizer for TTT adaptation
    ttt_optimizer = torch.optim.SGD(
        model.parameters(), lr=args.ttt_lr, momentum=args.ttt_momentum
    )
    cos_schedule = lambda epoch, total: 0.5 * (1 + math.cos(math.pi * epoch / total))

    for ci, cs in enumerate(chunk_starts):
        chunk_end = min(cs + chunk_size + 1, total_tokens + 1)
        chunk_tokens = val_tokens[cs:chunk_end].to(device=device, dtype=torch.int64)
        chunk_len = chunk_tokens.numel() - 1

        # Phase 1: Score this chunk under inference_mode (no gradients, no weight mutation)
        with torch.inference_mode():
            num_seqs = chunk_len // seq_len
            if num_seqs > 0:
                scored_len = num_seqs * seq_len
                x = chunk_tokens[:scored_len].reshape(num_seqs, seq_len)
                y = chunk_tokens[1:scored_len + 1].reshape(num_seqs, seq_len)
                # Process in mini-batches to avoid OOM
                batch_sz = max(1, min(16, num_seqs))
                for bi in range(0, num_seqs, batch_sz):
                    bx = x[bi:bi + batch_sz]
                    by = y[bi:bi + batch_sz]
                    with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                        ...
                        nll = F.cross_entropy(
                            ...
                            by.reshape(-1),
                            reduction="none",
                        ).reshape(bx.size(0), seq_len)
                    loss_sum += nll.to(torch.float64).sum()
                    token_count += float(bx.numel())
                    tgt = by.reshape(-1)
                    prev = bx.reshape(-1)
                    tb = base_bytes_lut[tgt].to(torch.float64)
                    tb += (has_leading_space_lut[tgt] & ~is_boundary_token_lut[prev]).to(torch.float64)
        ...
        # Phase 2: Adapt on this scored chunk (legal: already graded)
        # Skip adaptation on last chunk
        if ci < len(chunk_starts) - 1:
            model.train()
            for epoch in range(args.ttt_epochs):
                lr_scale = cos_schedule(epoch, args.ttt_epochs)
                for pg in ttt_optimizer.param_groups:
                    pg["lr"] = args.ttt_lr * lr_scale
                # Shuffle sequences within chunk for adaptation
                if num_seqs > 0:
                    perm = torch.randperm(num_seqs, device=device)
                    for bi in range(0, num_seqs, batch_sz):
                        idx = perm[bi:bi + batch_sz]
                        bx = x[idx]
                        by = y[idx]
                        ttt_optimizer.zero_grad()
                        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                            adapt_loss = model(bx, by)
                        adapt_loss.backward()
...
def main() -> None:
    ...
    log0(
        subprocess.run(["nvidia-smi"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, check=False, env={**os.environ, "PATH": os.environ.get("PATH", "") + ":/usr/lib/wsl/lib"}).stdout,
        console=False,
    )
    log0("=" * 100, console=False)
    ...
    log0(f"attn_res:{args.attn_res} attn_res_block_size:{args.attn_res_block_size}")
    log0(
    ...
    # Estimate total steps, use step-based warmdown with proportional scaling.
    # The original wallclock warmdown breaks on slow GPUs (warmdown from step 1).
    # warmdown_iters=3500 was tuned for ~9000 total steps (~39% warmdown).
    # We preserve that ratio on any hardware.
    estimated_total_steps: int | None = None
    def lr_mul(step: int, elapsed_ms: float) -> float:
        nonlocal estimated_total_steps
        ...
        if step > 0:
            ...
            estimated_total_steps = min(int(max_wallclock_ms / step_ms), args.iterations)
        if estimated_total_steps is None:
            return 1.0
        # Scale warmdown proportionally: keep same warmdown fraction
        warmdown_frac = min(args.warmdown_iters / max(args.iterations, 1), 0.5)
        effective_warmdown = max(int(estimated_total_steps * warmdown_frac), 1)
        warmdown_start = max(estimated_total_steps - effective_warmdown, 0)
        if step < warmdown_start:
            return 1.0
        return max((estimated_total_steps - step) / max(effective_warmdown, 1), 0.0)
    ...
    while True:
        ...
        # Progressive sequence length: ramp from prog_seq_start to train_seq_len
        if args.prog_seq and estimated_total_steps is not None and estimated_total_steps > 0:
            ramp_steps = int(args.prog_seq_ramp_frac * estimated_total_steps)
            if step < ramp_steps:
                # Linear ramp, snapped to multiples of 128 for efficiency
                frac_seq = step / max(ramp_steps, 1)
                cur_seq_len = int(args.prog_seq_start + frac_seq * (args.train_seq_len - args.prog_seq_start))
                cur_seq_len = max(128, (cur_seq_len // 128) * 128)
            else:
    ...
    if args.ttt_enabled:
        ...
        # Use a fresh copy of the quantized model for TTT (so we don't mutate the eval_model)
        ttt_model = copy.deepcopy(eval_model)
        ttt_model.train()
        ...
        log0(
            ...
            f"lr:{args.ttt_lr} epochs:{args.ttt_epochs} chunk:{args.ttt_chunk_tokens}"
        )
